...
<header class="dash-toolbar">
    ...
    <div class="main-label">
       <button id="add_new_btn">
        <i><img src="{{asset('icons/addbtn.svg')}}" style="width:20px; height:20px;"/></i> Add New Position
       </button>
...
@section('main')
                <p class="p1">Positions (<span id="usersnum">{{count($positions)}}</span>)</p>
                ...
                <div class="position_table">
                    <table class="pro-table" id="deptbl" style="padding-top:3%;">
					   ...
					   <tr>
					   <td>{{$d->part_project}}</td>
					    <td>{{$d->name}}</td>
					   <td><a href='/position/{{$d->id}}/edit' style='color:#377CDB;'><i> <img class="delete_img" src="{{ asset('icons/edit.svg') }}"> </i></a></td>
                       <td>  <a href="/admin/position/{{$d->id}}/destroy" style='color:#DC3545;'><i> <img class="delete_img" src="{{ asset('icons/delete.svg') }}"> </i></a></td>

					   @endforeach
                       <a class="btn btn-danger" href="{{route('position.trashed')}}"> Trash <i class="fas fa-trash"></i></a>
                    </tr>
                        </tbody>
                    </table>
                </div>
				@if(isset($position))
					@include('modal.position.positionEdit')
				@endif
					@include('modal.position.positionModal')
...
<script>
    ...
    $(document).ready(function(){
        ...
        var modal1 = document.getElementById("positionModal");
